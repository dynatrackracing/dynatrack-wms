<!-- SYNC STAMP -->
LAST PUSHED COMMIT: 265785d @ 2026-07-07 18:15 UTC (FIX DEPLOYED — returns re-ship bug closed [follows the 17:34 diagnosis]. server.js reconcile Phase-1 ON-CONFLICT now self-heals ship_move_applied_at when a SHIPPED line's matched item is ALREADY SHIPPED [COALESCE, never un-stamps; EXISTS on items; skips DISMISSED] → stops Phase 2 re-shipping a manually-shipped item on return-to-STORED, and stops new landmines. Did NOT touch Phase 2 / disposition / ship-wins-over-cancel; /api/move belt-and-suspenders deliberately skipped [Phase 1 fully closes it]. node --check OK; /api/health 200 db:connected post-deploy. One-time GATED backfill [dry-run→"commit", DATABASE_URL_RW]: stamped 608 unstamped SHIPPED lines to COALESCE(ebay_shipped_time,last_synced,NOW()); rollback artifact ~/hawker-ship-stamp-backfill-rollback-2026-07-07T18-14-20-202Z.json. VERIFY: 0 unstamped SHIPPED / 0 armed / 0 latent remain; MOD18130 untouched [STORED @ HR06S01]. Did NOT build the Returns page/schema [next deliverable]. OPEN carried: 🔐 ROTATE postgres password, addToBatch 409 UX, ZZ9001-3 verify-artifact purge, hyphen-insensitive moves history.)
STAMP UPDATED BY: Claude Code, session 18:15 UTC 2026-07-07
<!-- END SYNC STAMP -->
