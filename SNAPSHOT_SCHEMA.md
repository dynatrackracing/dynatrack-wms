...
**547 locations** (510 RACK + 35 SHELF_BIN + 1 UNLISTED_TOTE + 1 SHIPPED — migrations 0007–0008 2026-06-05: 0007 rack-pattern SHELF_BIN→RACK + 1 bin deleted + 9 section-O renamed; 0008 Section A's 20 `AR##S##` UNLISTED_TOTE→RACK; lone tote = `RETURN-1`) · **3,390 items** (all `status='STORED'`, `archived_at` NULL; **`intake_date` backfilled 2026-05-31** from the extract's `createdAt` — was NULL at cutover) · **3,390 moves** (all `moved_by='import-baseline'`) · 12 sequences (vestigial) · `ebay_order_lines` empty until the first post-cutover eBay sync. Live-inventory-only baseline (shipped items dropped — eBay/ShippingEasy own shipped now). Drastically different counts in a session = investigate before changing anything. *(Prior baseline 2026-04-02: 537 loc / 3,380 items / 3,969 moves — superseded by the cutover reload.)*
