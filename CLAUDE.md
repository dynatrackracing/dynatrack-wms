# ⚠️ PROJECT IDENTITY: HawkerWMS
This repo is HawkerWMS (github.com/dynatrackracing/dynatrack-wms,
Railway project robust-respect, hawkerwms.up.railway.app). It is NOT
the other eBay project. If you believe you are working on any other
project, STOP — you are in the wrong repo. Memory files for this
project are: CLAUDE.md, HAWKER_RULES.md, HAWKER_SESSION.md,
HAWKER_CHANGELOG.md.
...
1. Read HAWKER_RULES.md in full. Every rule is non-negotiable.
2. Read the most recent ~200 lines of HAWKER_SESSION.md. This tells you what state the project is in. Do not start work without this context.
3. Read the most recent ~100 lines of HAWKER_CHANGELOG.md.
...
G. **COMMIT THE WAY THE PROJECT DOES IT.** Per HAWKER_RULES rule 4: `git add -A && git commit -m "descriptive message" && git push origin main`. Always push. Both machines depend on the remote being current.

H. **UPDATE HAWKER_SESSION.md AT END OF SESSION.** Per HAWKER_RULES rule 5: append-only, with an `## HH:MM UTC — Description` header. State what was changed, what files were touched, what is still broken, what is next.

I. **UPDATE HAWKER_CHANGELOG.md AT END OF SESSION.** Per HAWKER_RULES rule 6.
...
K. **REGENERATE SNAPSHOTS AT SESSION END** when code changed this session. If you touched `server.js`, `public/index.html`, or `db/schema.sql`, regenerate the matching `SNAPSHOT_*.md` before committing (per HAWKER_RULES rule 38). Snapshots must reflect HEAD — never let them drift.

L. **REMIND THE HUMAN TO RE-UPLOAD PROJECT KNOWLEDGE** at session end if any of the four memory files were touched this session (per HAWKER_RULES rule 39).

M. **STATE YOUR SYNC STAMP FIRST (web architect).** At session start, read the SYNC STAMP block atop HAWKER_SESSION.md and announce the commit hash + push time to the human before any analysis or task work. If you cannot find a stamp, say so — it means the project knowledge predates Rule 40 and is almost certainly stale.
...
- eBay SKU normalization: eBay SKUs have trailing letter suffixes (`INT4306R`, `MOD19595V`); WMS serials are bare (`INT4306`). Strip trailing letters before matching. (See HAWKER_RULES rule 8.)
- Supabase is dead. Never reference it, never restore it. (See HAWKER_RULES rule 7.)
...
- **Canonical source = the git repo at `C:\Users\atenr\dynatrack-wms-repo`** (origin: `github.com/dynatrackracing/dynatrack-wms`, branch `main`). It holds the code (`server.js`, `public/index.html`, `db/`, …) **and** the four memory files (`CLAUDE.md`, `HAWKER_RULES.md`, `HAWKER_SESSION.md`, `HAWKER_CHANGELOG.md`) **and** the `SNAPSHOT_*.md` orientation files. Railway auto-deploys from `origin/main`.
